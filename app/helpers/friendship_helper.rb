...
module FriendshipHelper
  def follow_btn(id = nil, is_me = false)
    ...
    if is_me || @is_me
      out << if current_user.request?(id || params[:id])
               ...
               link_to('Add Friend', friendships_url(friend_id: (id || params[:id])), method: :post,
                                                                                      class: 'button is-pink mt-3')
             elsif friend.nil?
               '<button class="button is-info mt-3" title="Disabled button" disabled>Pending</button>'
             else
               link_to 'Unfriend', friendship_path(current_user.friendships.find_by_friend_id(@users.ids)),
                       method: :delete, class: 'button is-pink mt-3'
             end
    end
    out.html_safe
  end
  ...
  def list_friends
    ...
    @friends.each do |f|
      puts f
      out << "<li>#{link_to f.name, user_path(id: f.id),
                            class: 'has-text-pink is-size-5'} #{current_user.friend?(f.id).nil? ? ', pending' : ''}<li>"
    end
    out.html_safe
  end
  ...
  def accept_decline(user_id)
    ...
    out << "<div>
              ...
              #{link_to('Decline', friendship_url(id: user_id), method: :delete,
                                                                class: 'button is-danger ml-3 mt-3 px-5')}
          </div>"
    out.html_safe
  end
  ...
  def list_requests
    ...
    @requests.each do |f|
      out << "<li class=\"single-user mr-3 mb-3\">
                <strong class=\"has-text-white\">#{link_to f.name, user_path(id: f.id),
                                                           class: 'has-text-pink is-size-5'}</strong>
                #{accept_decline(f.id)}
            </li>"
    end
    out.html_safe
